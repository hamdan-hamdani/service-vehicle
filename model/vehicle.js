...
exports.getConditionVehicle = (condition) => {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT vehicle.*, category_detail.nameCategory FROM vehicle INNER JOIN category_detail ON vehicle.category = category_detail.id WHERE vehicle.name LIKE '%${condition.search}%' AND category_detail.nameCategory LiKE '%${condition.filter}%' ORDER BY ${condition.sort} ${condition.order} LIMIT ${condition.limit} OFFSET ${condition.offset}`,
      (err, result, field) => {
        if (err) {
          return reject(new Error(err));
        }
